...
func getMachineConfigPoolNameFromMachineOSConfigOrMachineOSBuild(mosb *mcfgv1.MachineOSBuild, mosc *mcfgv1.MachineOSConfig) (string, error) {
	...
	return getRequiredLabelValue(mosb, constants.TargetMachineConfigPoolLabelKey)
}
...
func getMachineOSBuildNameFromMachineOSConfigOrMachineOSBuild(mosb *mcfgv1.MachineOSBuild, mosc *mcfgv1.MachineOSConfig) (string, error) {
	...
	return getRequiredAnnotationValue(mosc, constants.CurrentMachineOSBuildAnnotationKey)
}
...
// getRequiredLabelValue returns the label value for the given key or an error.
func getRequiredLabelValue(obj metav1.Object, key string) (string, error) {
	val, ok := obj.GetLabels()[key]
	if !ok || val == "" {
		return "", fmt.Errorf("could not get label for object %s: required key %q missing", obj.GetName(), key)
	}
	return val, nil
}

// getRequiredAnnotationValue returns the annotation value for the given key or an error.
func getRequiredAnnotationValue(obj metav1.Object, key string) (string, error) {
	annos := obj.GetAnnotations()
	if annos == nil {
		return "", fmt.Errorf("could not get annotation for object %s: uninitialized map", obj.GetName())
	}
	val, ok := annos[key]
	if !ok || val == "" {
		return "", fmt.Errorf("could not get annotation for object %s: required key %q missing", obj.GetName(), key)
	}
	return val, nil
}
